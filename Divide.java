...
public class Divide {
	public double div(LinkedHashMap<String, String> variable, ArrayList<String> word,LinkedHashMap<String, String> all_list) {
		double temp = 0;
		for (int i = 2; i < word.size(); i++) {
			...
			if (see.matches("\\d+\\.{0,1}\\d*")) {
				if (i == 2) {
					...
					if (Double.parseDouble(see) == 0) {
						return -3;
					}
					temp = temp / Double.parseDouble(see);
				}
					...
					if (same.equals(see)) {
						...
						if (variable.get(same).matches("\\d+\\.{0,1}\\d*")) {
							if (i == 2) {
								...
								if (Double.parseDouble(variable.get(same)) == 0) {
									return -3;
								}
								temp = temp / Double.parseDouble(variable.get(same));
							}
						} else {
							return -2;
						}
